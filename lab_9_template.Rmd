...
# Yes, relationship appears linear for all three types. 
...
# a. Saturated model: 
homes_lm1 <- lm(Price ~ City + Bedrooms + Bathrooms + SqFt + PricePerSqFt + Status, data = homes_sub) # dependent variable is always on left side of equation

# Note: THIS IS BAD (because PricePerSqFt is calculated from the dependent variable Price --- biased overfitting of model results). It makes no sense to include a variable that is calculated from your output in order to predict the output variable.
# this is telling me that a house sold at regular sale status is gonna sell for less than the same house if its foreclosed on (not true). This also tels me that as you increase the house by 1 bedroom, the price goes down by $31,000 (not true).
# bedrooms, bathrooms, and sqft are all saying the same thing; how big the house is. Best to leave in your model is SqFt.
...
homes_lm2
# slope shows how much outcome variable will change if everything else stays the same. We expect house in SLO to sell for $34,525 more on average than a house in Arroyo Grande (reference level). I would expect a house in Atascadero on average to sell for $20,2157 less than a house in Arroyo Grande. StatusRegular = if you take two houses that are exactly the same, a house that sells in regular status, we expect it to sell on average $209,999 more than a house in foreclosure (reference level).

# b. Model summary:
summary(homes_lm2)
# short sale is not much different from foreclosure.
# p-value tells us that this model significantly predicts housing outcomes.
...
homes_sub$Status <- factor(homes_sub$Status) # now in R's brain, it's a factor instead of a character.
...
# the first one is always the reference level (Foreclosure)
...
# Residuals vs Fitted: vertical scatter isn't increasing much as price increases. Best house to buy would be the 188 (way below the price the model predicts).
# Normal Q-Q: don't be thrown off by a few outliers. Overall, data is normally distributed.
...
# you should never select a final model based strictly on the AIC
...
price_predict <- predict(homes_lm3, 
                         newdata = df_new, 
                         se.fit = TRUE, # what the error is associated with it
                         interval = "confidence") # what the confidence interval is around that mean value

# b. Bind predictions to the data to make it actually useful:
predict_df <- data.frame(df_new, price_predict) # data.frame lumps two columns together as long as they have the same # of rows
...
predict_graph <- ggplot(predict_df, aes(x = SqFt, y = fit.fit)) +
  ...
  facet_wrap(~Status) # can use faceting to show more than two variables in a way that is more digestible to an audience
